...
def alarm(bot, job):
    ...
    bot.send_message(job.context, text='BOOM!')
...
def set(bot, update, args, job_queue, chat_data):
    ...
    try:
        ...
        due = int(args[0])
        if due < 0:
            update.message.reply_text('Input time in seconds ... ok?!')
            return
        ...
        update.message.reply_text('Time Bomb successfully set!')
...
def unset(bot, update, chat_data):
    ...
    if 'job' not in chat_data:
        update.message.reply_text('You have no active Time Bomb')
        return
    ...
    update.message.reply_text('Defused!')
...
def main():
    updater = Updater("429075367:AAEpPNE2R6l1UDdBmn-dJnTuQBdNCE9XdZg")
